import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { Project } from 'ts-morph';
import { describe, expect, it } from 'vitest';
import { runRules } from '../engine/rule-engine.js';
import { preferShadcnAlert } from './prefer-shadcn-alert.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const fixturesDir = path.resolve(__dirname, '../__fixtures__');

describe('prefer-shadcn-alert rule', () => {
  const project = new Project();

  it('should detect <div> with role="alert"', () => {
    const filePath = path.join(fixturesDir, 'raw-html-elements.tsx');
    const sourceFile = project.addSourceFileAtPath(filePath);
    const { findings } = runRules(sourceFile, [preferShadcnAlert], fixturesDir);

    expect(findings).toHaveLength(1);
    expect(findings[0]).toMatchObject({
      rule: 'prefer-shadcn-alert',
      violation: 'Custom alert <div> detected. Use <Alert> from shadcn/ui.',
      element: 'div',
      replacement: 'Alert',
    });
  });

  it('should not detect shadcn/ui <Alert> component', () => {
    const filePath = path.join(fixturesDir, 'clean-component.tsx');
    const sourceFile = project.addSourceFileAtPath(filePath);
    const { findings } = runRules(sourceFile, [preferShadcnAlert], fixturesDir);

    expect(findings).toHaveLength(0);
  });
});
