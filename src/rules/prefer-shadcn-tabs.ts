...
export const preferShadcnTabs: Rule = {
  ...
  description: 'Detects custom tab-like button groups and suggests using shadcn/ui <Tabs> component.',
  ...
  check: (node: Node): Finding | null => {
    ...
    if (tagName === 'button') {
      ...
      const hasTabKeyword = classNameValue.toLowerCase().includes('tab') && !classNameValue.toLowerCase().includes('table');
